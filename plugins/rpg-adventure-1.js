...
const handler = async (m, {usedPrefix, conn}) => {
  try {
    ...
    if (user.health < 80) {
      return conn.reply(
          m.chat,
          `_${htki} صحتك منخفضة ${htka}_\n\n  ، يرجى الاعتناء بنفسك أولاً لتغامر مرة أخرى`,
          m,
      );
    }
    if (new Date() - user.lastadventure <= cooldown) {
      return conn.reply(
          m.chat,
          `${htki} خذ راحة ${htka}\n\nعليك اخذ راحة قبل المغامرة مرة ثانية\n\n⏱️ ${timers.toTimeString()} متبقي`,
          m,
      );
    }
    const rewards = reward(user);
    let text = `🛫 انت تغامر في *» ${kt[1][0].name}*

${cmenut}
${cmenub} *الاسم:* ${kt[1][0].id}
${cmenub} *المدينة:* ${kt[1][0].capitalCity}
${cmenub} *خطوط الطول:* ${kt[1][0].longitude}
${cmenub} *خطوط العرض:* ${kt[1][0].latitude}
${cmenuf}
...
${cmenua}`;
    ...
    conn.reply(m.chat, `${htki} مغامرة ${htka}\n\n${text}`, m);
...
handler.tags = ['rpg'];
handler.command = /^(adventure|adv|غامر|aventurar)$/i;
