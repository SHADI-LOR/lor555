...
const credit = `${pickRandom([300, 500, 700, 900, 500, 800, 900, 1100, 1350, 1500])}` * 1;
const creditpremium = `${pickRandom([800, 1300, 1600, 1900, 2200, 2500, 2700, 3000, 3300, 3500])}` * 1;
  ...
  const recompensas = {
   ...
   credit: premium ? creditpremium : credit,
  ...
  if (new Date - user.lastclaim < 7200000) return await conn.reply(m.chat, `لقد حصلت بالفعل على هديتك 🎁\nعد بعد *${msToTime(time - new Date())}* للاستلام مجددا`, fkontak, m);
  ...
  const text = `╭━━🎁━🎁━🎁━━⬣
┃ ✨ لقد حصلت على هدية!!
┃ *${premium ? '🎟️ مستخدم مميز' : '🆓 مستخدم عادي'}*
┃ ${texto}
╰━━🎁━🎁━🎁━━⬣\n\n🎟️ *مستخدم مميز! * ⇢ ${premium ? '✅' : '❌'}\n${wm}`;
  const img = './Menu.png';
...
handler.help = ['daily'];
handler.tags = ['xp'];
handler.command = ['d', 'reclamar', 'يومي', 'regalo', 'claim'];
