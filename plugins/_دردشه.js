...
export async function before(m, { conn, isAdmin, isBotAdmin, usedPrefix }) {
  ...
  let fakemek = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "51995386439-1616969743@g.us","inviteCode": "m","groupName": "P", "caption": 'ɢᴀᴛᴀʙᴏᴛ-ᴍᴅ', 'jpegThumbnail': null}}}
   if (chat.antiTraba && m.text.length > 4000) { //Cantidad máxima de caracteres aceptados en un mensaje//
    if (isAdmin) return conn.sendMessage(m.chat, { text: `${lenguajeGB['smsEnlaceWat']()}مسؤول @${m.sender.split("@")[0]} لقد أرسلت للتو نصًا يحتوي على العديد من الأحرف -.-!`, mentions: [m.sender] }, { quoted: fakemek })
    conn.sendMessage(m.chat, `*[ ! ] تم اكتشاف رسالة تحتوي على العديد من الأحرف [ ! ]*\n`, `${isBotAdmin ? '' : 'أنا لست مشرفًا ، ولا يمكنني فعل أي شيء :/'}`, m)

        if (isBotAdmin) {
        conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
          setTimeout(() => { 
          conn.sendMessage(m.chat, { text: `اجعل الدردشة مقروءة ✓\n${"\n".repeat(400)}\n• الرقم : wa.me/${m.sender.split("@")[0]}\n• الاسم : ${name}\n‼️لقد أرسلت للتو نصًا يحتوي على العديد من الأحرف التي يمكن أن تسبب فشل الجهاز`, mentions: [m.sender] }, { quoted: fakemek })
        }, 0)
        setTimeout(() => { 
