...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  try {
    ...
    if (filesDeleted === 0) {
      ...
      await conn.sendMessage(m.chat, {text: `*⟣⟤🛡️ تم حذف ${filesDeleted} ملف من الجلسه بتاعه البوت*`}, {quoted: m});
    }
  } catch (err) {
  ...
  await conn.sendMessage(m.chat, {text: `*👋 اهلا بك أتبع التعليمات*\n\n*⟣⟤⚡ اذا لم يرد البوت علي الامر الخاص بك يرجي وضع مثال صغير 3 مرات*\n\n*⟣⟤ مثال:*\n⟣⟤${usedPrefix}s\n⟣⟤${usedPrefix}⟣⟤s\n⟣⟤${usedPrefix}s`}, {quoted: m});
};
handler.help = ['fixmsgespera'];
handler.tags = ['fix'];
handler.command = /^(تصليح|ds|صلح)$/i;
