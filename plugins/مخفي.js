...
let handler = async (m, { conn, text, participants }) => {
  try {
    ...
    }, { quoted: m, userJid: conn.user.id }), text || q.text, conn.user.jid, { mentions: users })
    ...
    let htextos = `${text ? text : "سلام وعليكم !!"}`
    if ((isMedia && quoted.mtype === 'imageMessage') && htextos) {
      ...
      await conn.relayMessage(m.chat, { extendedTextMessage: { text: `${masss}${htextos}\n`, ...{ contextInfo: { mentionedJid: users } } } }, {})
    }
  }
}
