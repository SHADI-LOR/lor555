...
if (!args[0]) return conn.reply(m.chat, `${lenguajeGB['smsAvisoMG']()}_*فين رابط الفيديو الي عايز تحمله من الفيسبوك*_\n_*مثل*_\n*${usedPrefix + command} https://www.facebook.com/watch?v=636541475139`, fkontak, m)
if (!args[0].match(/www.facebook.com|fb.watch/g)) return conn.reply(m.chat, `${lenguajeGB['smsAvisoMG']()}_*فين رابط الفيديو الي عايز تحمله من الفيسبوك*_\n_*مثل*_\n*${usedPrefix + command} https://www.facebook.com/watch?v=636541475139`, fkontak, m)
try {
await conn.reply(m.chat, `${lenguajeGB['smsAvisoEG']()}_*انتظر حتي يتم التحميل ⌛⌛*_`, fkontak, m)
const d2ata = await facebook.v1(args[0]);
let r2es = '';
if (d2ata.urls && d2ata.urls.length > 0) {
r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`
}
conn.sendFile(m.chat, r2es, 'error.mp4', `*✅ تـم التحميل*\n${wm}`, m);
} catch (err1) {
try {
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `*✅ تـم التحميل*\n${wm}`, m);
} catch (err2) {
try {
const ress = await fg.fbdl(args[0]);
const urll = await ress.data[0].url;
await conn.sendFile(m.chat, urll, 'error.mp4', `*✅ تـم التحميل*\n${wm}`, m);
} catch (err3) {
try {
const res = await fbDownloader(args[0]);
for (const result of res.download) {
const ur = result.url;
await conn.sendFile(m.chat, ur, 'error.mp4', `*✅ تـم التحميل*\n${wm}`, m);
}
} catch (err4) {
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', `*✅ تـم التحميل*\n${wm}`, m);
} catch (err5) {
try {
const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, `*✅ تـم التحميل*\n${wm}`, m);
} catch (err6) {
await m.reply(`${lenguajeGB['smsAvisoFG']()} _*انتظر حتي يتم التحميل ⌛⌛*_`)
handler.limit = 0
}}}}}}}}
handler.command = /^(facebook|fb|facebookdl|fbdl|facebook2|fb2|facebookdl2|fbdl2|facebook3|fb3|facebookdl3|fbdl3|facebook4|fb4|facebookdl4|fbdl4|فيس|fb5|فيسبوك|fbdl5)$/i
handler.limit = 3
...
handler.command = /^(facebook|fimport fg from 'api-dylux';
import fetch from 'node-fetch';
import {savefrom, facebookdl, facebookdlv2} from '@bochilteam/scraper';
import fbDownloader from 'fb-downloader-scrapper';
import {facebook} from '@xct007/frieren-scraper';
...
handler.limit = 3
export default handler*/|facebookdl|fbdl|facebook2|fb2|facebookdl2|fbdl2|facebook3|fb3|facebookdl3|fbdl3|facebook4|fb4|facebookdl4|fbdl4|facebook5|fb5|facebookdl5|fbdl5)$/i
handler.limit = 3
export default handler*/
