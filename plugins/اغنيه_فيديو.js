...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `*⟣⟤⚡⟥⟢*\n*حط اسم عشان تنزل اللي انت عايزو*\n\n*⟣⟤ مثل:*\n*${usedPrefix + command} xxxtntacion sad*\n~*انت المسؤل كامل المسؤولية عن ذنوبك~*`;
  try {
    const yt_play = await search(args.join(' '));
    let additionalText = '';
    if (command === 'play') {
      additionalText = 'الاغنيه 🔊';
    } else if (command === 'play2') {
      additionalText = 'الفيديو 🎥';
    }
    const texto1 = ` *_${yt_play[0].title}_*
*⇄ㅤ     ◁   ㅤ  ❚❚ㅤ     ▷ㅤ     ↻*
            ...
            try {
              ...
              try {
                ...
                try {
                  ...
                  await conn.reply(m.chat, '*[❗] حصل خطأ ابلغ ڤينوم او سوكونا به*', m);
                }
              }
            }
          }
    ...
    if (command == 'فيديو') {
      try {
        ...
        try {
          ...
          await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `_𝐕𝐄𝐍𝐎𝐌 𝐀𝐍𝐃 𝐒𝐎𝐔𝐊𝐍𝐀_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
        } catch {
          try {
