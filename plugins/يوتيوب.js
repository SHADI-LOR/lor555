...
if (!args[0]) return conn.reply(m.chat, `${lenguajeGB['smsAvisoMG']()}_*نسيت تحط الرابط الي عايز تنزله*_\n_*مثل*_\n*${usedPrefix + command} https://youtu.be/85xI8WFMIUY*`, fkontak, m)
...
throw `${lenguajeGB['smsAvisoFG']()}تستعمل الامر بطريقة غير صحيحة${matchingItem.urls.length}*`;
}} else {
throw `${lenguajeGB['smsAvisoMG']()} تستعل الامر بطريقه غير صحيحة`;
}} else {
throw `${lenguajeGB['smsAvisoMG']()}  تستعل الامر بطريقه غير صحيحة`;
}}}  

await conn.reply(m.chat, `${lenguajeGB['smsAvisoEG']()}يتم التحميل من اليوتيوب انتظر قليلاً فقط`, fkontak, m)
try {
let qu = args[1] || '360'
let q = qu + 'p'
let v = youtubeLink
...
await conn.reply(m.chat, `${lenguajeGB['smsAvisoFG']()}تاكد..........`, m)}
}}}
handler.command = /^video|fgmp4|dlmp4|يوتيوب|yt(v|mp4)?$/i
