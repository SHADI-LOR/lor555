...
//المعلمات في الأوامر
const smsRowner = () => { return `\`\`\`¡¡هذا الأمر أنا لصانع الروبوت فقط!!\`\`\`` }
const smsOwner = () => { return `\`\`\`¡¡هذا الأمر أنا لصانع الروبوت فقط!!\`\`\`` }
const smsMods = () => { return `\`\`\`¡¡هذا الأمر أنا لصانع الروبوت فقط!!\`\`\`` }
const smsPremium = () => { return `\`\`\`¡¡يتوفر هذا الأمر فقط للمستخدمين المتميزين ومنشئ المحتوى الخاص بي!! للحصول على قسط شراء تذكرة باستخدام #شراء بريم\`\`\`` }
const smsGroup = () => { return `\`\`\`¡¡لا يمكن استخدام هذا الأمر إلا في مجموعات!!\`\`\`` }
const smsPrivate = () => { return `\`\`\`¡¡لا يمكن استخدام هذا الأمر إلا في البرايفت!!\`\`\`` }
...
const smsUnreg = () => { return `\`\`\`¡¡أنت بحاجة إلى التسجيل لاستخدام هذا الأمر ، نوع #تسجيل للتسجيل!!\`\`\`` }
...
const smsLista1 = () => { return `🌟 معلومات ڤينوم و سوكونا🌟`}
...
const smsLista7 = () => { return `🐈 تثبيت ڤينوم و سوكونا 🐈`}
const smsLista8 = () => { return `🍄 كن بوت فرعي 🍄`}
...
//_allantilink.js
const smsTextoYT = () => { return '😻 ممتاز AnimeBot-MD - WhatsApp '}
...
const smsCont6 = () => { return `*❗ أبلغ عن هذه الرسالة باستخدام الأمر #ابلاغ من أجل حلها*`}
const smsCont7 = () => { return `${global.lenguajeGB['smsAvisoAG']()}*ليس لديك/ا ماسات!! 💎 يمكنك الذهاب إلى المتجر باستخدام الأمر*`}
...
const smsModAd2 = () => { return `سيتمكن المسؤولون فقط من استخدام animebot في المجموعات`}
...
const smsContacto4 = () => { return 'جهة اتصال animebot الرسمية 🐈'}
